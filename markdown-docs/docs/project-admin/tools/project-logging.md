...
Only [administrators](./admin-settings/rbac/global-permissions.md), [cluster owners or members](./admin-settings/rbac/cluster-project-roles/#cluster-roles), or [project owners](./admin-settings/rbac/cluster-project-roles/#project-roles) can configure Rancher to send Kubernetes logs to a logging service.

# Requirements

The Docker daemon on each node in the cluster should be [configured](https://docs.docker.com/config/containers/logging/configure.md) with the (default) log-driver: `json-file`. You can check the log-driver by running the following command:
...
- [Cluster logging](./cluster-admin/tools/logging.md) writes logs for every pod in the cluster, i.e. in all the projects. For [RKE clusters](./cluster-provisioning/rke-clusters), it also writes logs for all the Kubernetes system components.
...
1. Select a logging service and enter the configuration. Refer to the specific service for detailed configuration. Rancher supports the following services:
   ...
   - [Elasticsearch](./cluster-admin/tools/logging/elasticsearch.md)
   - [Splunk](./cluster-admin/tools/logging/splunk.md)
   - [Kafka](./cluster-admin/tools/logging/kafka.md)
   - [Syslog](./cluster-admin/tools/logging/syslog.md)
   - [Fluentd](./cluster-admin/tools/logging/fluentd.md)
...
1. (Optional) Complete the **Additional Logging Configuration** form.
    ...
    1. Enter a **Flush Interval**. This value determines how often [Fluentd](https://www.fluentd.org.md) flushes data to the logging server. Intervals are measured in seconds.
...
[Logging Architecture](https://kubernetes.io/docs/concepts/cluster-administration/logging.md)
