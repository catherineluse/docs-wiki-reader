...
When [adding your catalog](./catalog/custom/adding.md) to Rancher, you'll provide the following information:
...
1. When [adding the catalog](./catalog/custom/adding.md), select the **Use private catalog** checkbox.

2. Provide the `Username` and `Password` for your Git or Helm repository.

### Using an OAuth token

Read [using Git over HTTPS and OAuth](https://github.blog/2012-09-21-easier-builds-and-deployments-using-git-over-https-and-oauth.md) for more details on how OAuth authentication works.

1. Create an [OAuth token](https://github.com/settings/tokens)
with `repo` permission selected, and click **Generate token**.

2. When [adding the catalog](./catalog/custom/adding.md), select the **Use private catalog** checkbox.
