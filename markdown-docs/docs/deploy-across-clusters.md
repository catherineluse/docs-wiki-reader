...
After creating a multi-cluster application, you can program a [Global DNS entry](./helm-charts/globaldns.md) to make it easier to access the application.
...
- A [project-member role](./admin-settings/rbac/cluster-project-roles/#project-roles) in the target cluster(s), which gives you the ability to create, read, update, and delete the workloads
- A [cluster owner role](./admin-settings/rbac/cluster-project-roles/#cluster-roles) for the clusters(s) that include the target project(s)
...
- **Project** - This is the equivalent of a [project member](./admin-settings/rbac/cluster-project-roles/#project-roles). If you select this role, Rancher will check that in all the target projects, the user has minimally the [project member](./admin-settings/rbac/cluster-project-roles/#project-roles) role. While the user might not be explicitly granted the _project member_ role, if the user is an [administrator](./admin-settings/rbac/global-permissions.md), a [cluster owner](./admin-settings/rbac/cluster-project-roles/#cluster-roles), or a [project owner](./admin-settings/rbac/cluster-project-roles/#project-roles), then the user is considered to have the appropriate level of permissions.

- **Cluster** - This is the equivalent of a [cluster owner](./admin-settings/rbac/cluster-project-roles/#cluster-roles). If you select this role, Rancher will check that in all the target projects, the user has minimally the [cluster owner](./admin-settings/rbac/cluster-project-roles/#project-roles) role. While the user might not be explicitly granted the _cluster owner_ role, if the user is an [administrator](./admin-settings/rbac/global-permissions.md), then the user is considered to have the appropriate level of permissions.
...
For native Helm charts (i.e., charts from the **Helm Stable** or **Helm Incubator** catalogs or a [custom Helm chart repository](./catalog/custom.md), answers are provided as key value pairs in the **Answers** section. These answers are used to override the default values.
