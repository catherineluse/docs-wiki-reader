...
- The cloud provider must be enabled. For details on enabling cloud providers, refer to [this page.](./cluster-provisioning/rke-clusters/options/cloud-providers.md)
...
To use a storage provisioner that is not on the above list, you will need to use a [feature flag to enable unsupported storage drivers.](./installation/options/feature-flags/enable-not-default-storage-drivers.md)
...
1. Create a workload as you would in [Deploying Workloads](./k8s-in-rancher/workloads/deploy-workloads.md).
