...
After you've enabled monitoring at either the [cluster level](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md) or [project level](./project-admin/tools/monitoring.md), you will want to be start viewing the data being collected. There are multiple ways to view this data.

## Rancher Dashboard

>**Note:** This is only available if you've enabled monitoring at the [cluster level](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md). Project specific analytics must be viewed using the project's Grafana instance.
...
If you've enabled monitoring at either the [cluster level](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md) or [project level](./project-admin/tools/monitoring.md), Rancher automatically creates a link to Grafana instance. Use this link to view monitoring data.
...
Rancher determines which users can access the new Grafana instance, as well as the objects they can view within it, by validating them against the user's [cluster or project roles](./admin-settings/rbac/cluster-project-roles.md). In other words, a user's access in Grafana mirrors their access in Rancher.
