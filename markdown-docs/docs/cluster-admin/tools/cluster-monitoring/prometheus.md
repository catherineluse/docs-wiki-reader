...
While configuring monitoring at either the [cluster level](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md) or [project level](./project-admin/tools/monitoring.md), there are multiple options that can be configured.
...
Since monitoring is an [application](https://github.com/rancher/system-charts/tree/dev/charts/rancher-monitoring) from the [Rancher catalog](./catalog.md), it can be configured like any other catalog application, by passing in values to Helm.
...
>**Prerequisite:** Configure one or more StorageClasses to use as [persistent storage](./cluster-admin/volumes-and-storage.md) for your Prometheus or Grafana pod.
...
>**Prerequisite:** Need a remote storage endpoint to be available. The possible list of integrations is available [here](https://prometheus.io/docs/operating/integrations.md)
