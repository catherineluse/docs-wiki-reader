...
- Cluster logging writes logs for every pod in the cluster, i.e. in all the projects. For [RKE clusters](./cluster-provisioning/rke-clusters), it also writes logs for all the Kubernetes system components.
- [Project logging](./project-admin/tools/logging.md) writes logs for every pod in that particular project.
...
As an [administrator](./admin-settings/rbac/global-permissions.md) or [cluster owner](./admin-settings/rbac/cluster-project-roles/#cluster-roles), you can configure Rancher to send Kubernetes logs to a logging service.
