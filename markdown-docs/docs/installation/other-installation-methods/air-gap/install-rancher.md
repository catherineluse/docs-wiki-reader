...
2. Use `helm repo add` command to add the Helm chart repository that contains charts to install Rancher. For more information about the repository choices and which is best for your use case, see [Choosing a Version of Rancher](./installation/install-rancher-on-k8s/chart-options/#helm-chart-repositories).
  {{< release-channel >}}
    ```
...
> **Note:**
> Recent changes to cert-manager require an upgrade. If you are upgrading Rancher and using a version of cert-manager older than v0.11.0, please see our [upgrade cert-manager documentation](./installation/options/upgrading-cert-manager.md).
...
Then refer to [Adding TLS Secrets](./installation/resources/encryption/tls-secrets.md) to publish the certificate files so Rancher and the ingress controller can use them.
...
If you are installing Rancher versions before v2.3.0, you will not be able to use the packaged system charts. Since the Rancher system charts are hosted in Github, an air gapped installation will not be able to access these charts. Therefore, you must [configure the Rancher system charts](./installation/resources/local-system-charts.md).

# Additional Resources

These resources could be helpful when installing Rancher:

- [Rancher Helm chart options](./installation/resources/chart-options.md)
- [Adding TLS secrets](./installation/resources/encryption/tls-secrets.md)
- [Troubleshooting Rancher Kubernetes Installations](./installation/options/troubleshooting.md)
...
> **Do you want to...**
>
> - Configure custom CA root certificate to access your services? See [Custom CA root certificate](./installation/options/custom-ca-root-certificate.md).
> - Record all transactions with the Rancher API? See [API Auditing](./installation/other-installation-methods/single-node-docker/advanced/#api-audit-log).

- For Rancher before v2.3.0, you will need to mirror the `system-charts` repository to a location in your network that Rancher can reach. Then, after Rancher is installed, you will need to configure Rancher to use that repository. For details, refer to the documentation on [setting up the system charts for Rancher before v2.3.0.](./installation/resources/local-system-charts.md)
...
| `<RANCHER_VERSION_TAG>`          | The release tag of the [Rancher version](./installation/resources/chart-options.md) that you want to install. |
...
> **Prerequisites:**
> From a computer with an internet connection, create a self-signed certificate using [OpenSSL](https://www.openssl.org.md) or another method of your choice.
>
> - The certificate files must be in PEM format.
> - In your certificate file, include all intermediate certificates in the chain. Order your certificates with your certificate first, followed by the intermediates. For an example, see [Certificate Troubleshooting.](./installation/other-installation-methods/single-node-docker/troubleshooting)
