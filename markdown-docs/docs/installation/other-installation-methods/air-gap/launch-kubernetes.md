...
This section describes how to install a Kubernetes cluster according to our [best practices for the Rancher server environment.](./overview/architecture-recommendations/#environment-for-kubernetes-installations) This cluster should be dedicated to run only the Rancher server.

For Rancher before v2.4, Rancher should be installed on an [RKE](./rke/latest/en.md) (Rancher Kubernetes Engine) Kubernetes cluster. RKE is a CNCF-certified Kubernetes distribution that runs entirely within Docker containers.
...
For more information on private registries configuration file for K3s, refer to the [K3s documentation.](./k3s/latest/en/installation/private-registry.md)

### 3. Install K3s

Rancher needs to be installed on a supported Kubernetes version. To find out which versions of Kubernetes are supported for your Rancher version, refer to the [support maintenance terms.](https://rancher.com/support-maintenance-terms.md)
...
For more information about the `kubeconfig` file, refer to the [K3s documentation](./k3s/latest/en/cluster-access.md) or the [official Kubernetes documentation](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig.md) about organizing cluster access using `kubeconfig` files.
...
Install RKE by following the instructions in the [RKE documentation.](./rke/latest/en/installation.md)
...
> **Tip:** For more details on the options available, see the RKE [Config Options](./rke/latest/en/config-options.md).
...
- `kube_config_rancher-cluster.yml`: The [Kubeconfig file](./rke/latest/en/kubeconfig.md) for the cluster, this file contains credentials for full access to the cluster.
- `rancher-cluster.rkestate`: The [Kubernetes Cluster State file](./rke/latest/en/installation/#kubernetes-cluster-state), this file contains the current state of the cluster including the RKE configuration and the certificates.<br/><br/>_The Kubernetes Cluster State file is only created when using RKE v0.2.0 or higher._
{{% /tab %}}
{{% /tabs %}}
...
See the [Troubleshooting](./installation/options/troubleshooting.md) page.
