...
Use `helm repo add` command to add the Helm chart repository that contains charts to install Rancher. For more information about the repository choices and which is best for your use case, see [Choosing a Version of Rancher](./installation/resources/choosing-version).
...
| [Let’s Encrypt](#let-s-encrypt) | `ingress.tls.source=letsEncrypt` | Use [Let's Encrypt](https://letsencrypt.org.md) to issue a certificate | [yes](#optional-install-cert-manager) |
...
> Recent changes to cert-manager require an upgrade. If you are upgrading Rancher and using a version of cert-manager older than v0.12.0, please see our [upgrade documentation](./installation/options/upgrading-cert-manager.md).
...
This option uses `cert-manager` to automatically request and renew [Let's Encrypt](https://letsencrypt.org.md) certificates. This is a free service that provides you with a valid certificate as Let's Encrypt is a trusted CA. This configuration uses HTTP validation (`HTTP-01`) so the load balancer must have a public DNS record and be accessible from the internet.
...
Now that Rancher is deployed, see [Adding TLS Secrets](./installation/options/helm2/helm-rancher/tls-secrets.md) to publish the certificate files so Rancher and the ingress controller can use them.
...
* [HTTP Proxy](./installation/install-rancher-on-k8s/chart-options.md)
...
See the [Chart Options](./installation/options/helm2/helm-rancher/chart-options.md) for the full list of options.
...
Doesn't work? Take a look at the [Troubleshooting](./installation/options/helm2/helm-rancher/troubleshooting.md) Page
