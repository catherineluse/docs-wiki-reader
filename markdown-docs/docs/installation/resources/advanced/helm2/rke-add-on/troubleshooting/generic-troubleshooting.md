...
Double check if all the [required ports](./cluster-provisioning/node-requirements/#networking-requirements) are opened in your (host) firewall.
...
If you see error in the output, that means that the [required ports](./cluster-provisioning/node-requirements/#networking-requirements) for overlay networking are not opened between the hosts indicated.
