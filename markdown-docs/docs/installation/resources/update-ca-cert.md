...
Follow these steps to update the SSL certificate of the ingress in a Rancher [high availability Kubernetes installation](./installation/install-rancher-on-k8s.md) or to switch from the default self-signed certificate to a custom certificate.
...
If the clusters have lost connection to Rancher but you have [Authorized Cluster Endpoints](https://rancher.com/docs/rancher/v2.0-v2.4/en/cluster-admin/cluster-access/ace.md) enabled, then go with method 2.
