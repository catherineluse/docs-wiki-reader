...
The port requirements differ depending on how the downstream cluster was launched. Each of the tabs below list the ports that need to be opened for different [cluster types](./cluster-provisioning.md).

The following diagram depicts the ports that are opened for each [cluster type](./cluster-provisioning).
...
The following table depicts the port requirements for [Rancher Launched Kubernetes](./cluster-provisioning/rke-clusters.md) with nodes created in an [Infrastructure Provider](./cluster-provisioning/rke-clusters/node-pools.md).
...
The following table depicts the port requirements for [Rancher Launched Kubernetes](./cluster-provisioning/rke-clusters.md) with [Custom Nodes](./cluster-provisioning/rke-clusters/custom-nodes.md).
...
The following table depicts the port requirements for [hosted clusters](./cluster-provisioning/hosted-kubernetes-clusters).
...
The following table depicts the port requirements for [imported clusters](./cluster-provisioning/imported-clusters.md).
...
When using the [AWS EC2 node driver](./cluster-provisioning/rke-clusters/node-pools/ec2.md) to provision cluster nodes in Rancher, you can choose to let Rancher create a security group called `rancher-nodes`. The following rules are automatically added to this security group.
