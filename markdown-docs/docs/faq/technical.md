...
See [Troubleshooting: Logging](./troubleshooting/logging.md)
...
When the IP address of the node changed, Rancher lost connection to the node, so it will be unable to clean the node properly. See [Cleaning cluster nodes](./faq/cleaning-cluster-nodes.md) to clean the node.
...
You can add additional arguments/binds/environment variables via the [Config File](./cluster-provisioning/rke-clusters/options/#cluster-config-file) option in Cluster Options. For more information, see the [Extra Args, Extra Binds, and Extra Environment Variables](./rke/latest/en/config-options/services/services-extras.md) in the RKE documentation or browse the [Example Cluster.ymls](./rke/latest/en/example-yamls.md).
...
See [Kubernetes: kubelet](https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet.md) and [Kubernetes: kube-controller-manager](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-controller-manager.md) for more information on these settings.
