...
Within Rancher, only administrators or users with the **Manage Authentication** [global role](./admin-settings/rbac/global-permissions.md) can configure authentication.
...
1. Sign into Rancher using a local user assigned the [administrator](./admin-settings/rbac/global-permissions) role. This user is also called the local principal.
