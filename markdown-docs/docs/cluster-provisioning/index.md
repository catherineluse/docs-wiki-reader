...
For a conceptual overview of how the Rancher server provisions clusters and what tools it uses to provision them, refer to the [architecture](./overview/architecture.md) page.
...
For more information, refer to the section on [hosted Kubernetes clusters.](./cluster-provisioning/hosted-kubernetes-clusters)

# Launching Kubernetes with Rancher

Rancher uses the [Rancher Kubernetes Engine (RKE)](./rke/latest/en.md) as a library when provisioning Kubernetes on your own nodes. RKE is Rancher’s own lightweight Kubernetes installer.
...
These nodes can be dynamically provisioned through Rancher's UI, which calls [Docker Machine](https://docs.docker.com/machine.md) to launch nodes on various cloud providers.
...
For more information, refer to the section on [RKE clusters.](./cluster-provisioning/rke-clusters.md)
...
Using Rancher, you can create pools of nodes based on a [node template](./cluster-provisioning/rke-clusters/node-pools/#node-templates). This template defines the parameters used to launch nodes in your cloud providers.
...
The cloud providers available for creating a node template are decided based on the [node drivers](./cluster-provisioning/rke-clusters/node-pools/#node-drivers) active in the Rancher UI.

For more information, refer to the section on [nodes hosted by an infrastructure provider](./cluster-provisioning/rke-clusters/node-pools.md)

### Launching Kubernetes on Existing Custom Nodes

When setting up this type of cluster, Rancher installs Kubernetes on existing [custom nodes,](./cluster-provisioning/rke-clusters/custom-nodes.md) which creates a custom cluster.
...
For more information, refer to the section on [importing existing clusters.](./cluster-provisioning/imported-clusters.md)

### Importing and Editing K3s Clusters

_Available as of Rancher v2.4.0_

[K3s](./k3s/latest/en.md) is a lightweight, fully compliant Kubernetes distribution. K3s Kubernetes clusters can now be imported into Rancher.
...
For more information, refer to the section on [imported K3s clusters.](./cluster-provisioning/imported-clusters.md)