...
>
>See [Configuring Custom Clusters for Windows](./cluster-provisioning/rke-clusters/windows-clusters.md) before you start.
...
Provision the host according to the [installation requirements](./cluster-provisioning/node-requirements) and the [checklist for production-ready clusters.](./cluster-provisioning/production)
...
5. Use **Cluster Options** to choose the version of Kubernetes, what network provider will be used and if you want to enable project network isolation. To see more cluster options, click on **Show advanced options.**
    ...
    >
    >- See [Enable the Windows Support Option](./cluster-provisioning/rke-clusters/windows-clusters.md).
...
7.	From **Node Role**, choose the roles that you want filled by a cluster node.
	...
	>**Notes:**
	>
    >- Using Windows nodes as Kubernetes workers? See [this section](./cluster-provisioning/rke-clusters/windows-clusters.md).
	>- Bare-Metal Server Reminder: If you plan on dedicating bare-metal servers to each role, you must provision a bare-metal server for each role (i.e. provision multiple bare-metal servers).

8.	<a id="step-8"></a>**Optional**: Click **[Show advanced options](./admin-settings/agent-options.md)** to specify IP address(es) to use when registering the node, override the hostname of the node, or to add [labels](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels.md) or [taints](https://kubernetes.io/docs/concepts/configuration/taint-and-toleration.md) to the node.
...
>**Note:** You can use Amazon EC2 instances without configuring a cloud provider in Kubernetes. You only have to configure the cloud provider if you want to use specific Kubernetes cloud provider functionality. For more information, see [Kubernetes Cloud Providers](https://kubernetes.io/docs/concepts/cluster-administration/cloud-providers.md)
