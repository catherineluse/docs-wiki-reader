...
When Rancher installs Kubernetes, it uses [RKE](./cluster-provisioning/rke-clusters.md) as the Kubernetes distribution.
...
In Rancher v2.0.0-v2.2.x, the RKE cluster config file in Rancher is identical to the  [cluster config file for the Rancher Kubernetes Engine](./rke/latest/en/config-options.md), which is the tool Rancher uses to provision clusters. In Rancher v2.3.0, the RKE information is still included in the config file, but it is separated from other options, so that the RKE cluster config options are nested under the `rancher_kubernetes_engine_config` directive. For more information, see the section about the [cluster config file.](#cluster-config-file)
...
When creating a cluster using one of the options described in [Rancher Launched Kubernetes](./cluster-provisioning/rke-clusters), you can configure basic Kubernetes options using the **Cluster Options** section.
...
The [Network Provider](https://kubernetes.io/docs/concepts/cluster-administration/networking.md) that the cluster uses. For more details on the different networking providers, please view our [Networking FAQ](./faq/networking/cni-providers.md).
...
- [Calico](https://docs.projectcalico.org/v3.11/introduction.md)
- [Weave](https://github.com/weaveworks/weave) (Available as of v2.2.0)

**Notes on Canal:**
  
In v2.0.0 - v2.0.4 and v2.0.6, this was the default option for these clusters was Canal with network isolation. With the network isolation automatically enabled, it prevented any pod communication between [projects](./k8s-in-rancher/projects-and-namespaces.md).

As of v2.0.7, if you use Canal, you also have the option of using **Project Network Isolation**, which will enable or disable communication between pods in different [projects](./k8s-in-rancher/projects-and-namespaces.md).
...
There are two main ways to set up private registries in Rancher: by setting up the [global default registry](./admin-settings/config-private-registry) through the **Settings** tab in the global view, and by setting up a private registry in the advanced options in the cluster-level settings. The global default registry is intended to be used for air-gapped setups, for registries that do not require credentials. The cluster-level private registry is intended to be used in all setups in which the private registry requires credentials.
...
The private registry configuration option tells Rancher where to pull the [system images](./rke/latest/en/config-options/system-images.md) or [addon images](./rke/latest/en/config-options/add-ons.md) that will be used in your cluster.
...
See the [RKE documentation on private registries](./rke/latest/en/config-options/private-registries.md) for more information on the private registry for components applied during the provisioning of the cluster.
...
> The authorized cluster endpoint only works on Rancher-launched Kubernetes clusters. In other words, it only works in clusters where Rancher [used RKE](./overview/architecture/#tools-for-provisioning-kubernetes-clusters) to provision the cluster. It is not available for clusters in a hosted Kubernetes provider, such as Amazon's EKS.

This is enabled by default in Rancher-launched Kubernetes clusters, using the IP of the node with the `controlplane` role and the default Kubernetes self signed certificates.

For more detail on how an authorized cluster endpoint works and why it is used, refer to the [architecture section.](./overview/architecture/#4-authorized-cluster-endpoint)
...
For information on using the Rancher UI to set up node pools in an RKE cluster, refer to [this page.](./cluster-provisioning/rke-clusters/node-pools)
...
Option to enable or disable the [NGINX ingress controller](./rke/latest/en/config-options/add-ons/ingress-controllers.md).
...
Option to enable or disable [Metrics Server](./rke/latest/en/config-options/add-ons/metrics-server.md).

### Pod Security Policy Support

Option to enable and select a default [Pod Security Policy](./admin-settings/pod-security-policies). You must have an existing Pod Security Policy configured before you can use this option.

### Docker Version on Nodes

Option to require [a supported Docker version](./installation/requirements.md) installed on the cluster nodes that are added to the cluster, or to allow unsupported Docker versions installed on the cluster nodes.
...
Option to enable or disable [recurring etcd snapshots](./rke/latest/en/etcd-snapshots/#etcd-recurring-snapshots).

# Cluster Config File

Instead of using the Rancher UI to choose Kubernetes options for the cluster, advanced users can create an RKE config file. Using a config file allows you to set any of the [options available](./rke/latest/en/config-options.md) in an RKE installation, except for `system_images` configuration. The `system_images` option is not supported when creating a cluster with the Rancher UI or API.
...
The table below indicates what DNS provider is deployed by default. See [RKE documentation on DNS provider](./rke/latest/en/config-options/add-ons/dns.md) for more information how to configure a different DNS provider. CoreDNS can only be used on Kubernetes v1.12.0 and higher.
...
Option to enable or disable [Cluster Monitoring](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md).
...
You can add a custom network plug-in by using the [user-defined add-on functionality](./rke/latest/en/config-options/add-ons/user-defined-add-ons.md) of RKE. You define any add-on that you want deployed after the Kubernetes cluster is deployed.

There are two ways that you can specify an add-on:

- [In-line Add-ons](./rke/latest/en/config-options/add-ons/user-defined-add-ons/#in-line-add-ons)
- [Referencing YAML Files for Add-ons](./rke/latest/en/config-options/add-ons/user-defined-add-ons/#referencing-yaml-files-for-add-ons)

For an example of how to configure a custom network plug-in by editing the `cluster.yml`, refer to the [RKE documentation.](./rke/latest/en/config-options/add-ons/network-plugins/custom-network-plugin-example)
