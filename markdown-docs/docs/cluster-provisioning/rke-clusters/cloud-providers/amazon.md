...
While creating an [Amazon EC2 cluster](./cluster-provisioning/rke-clusters/node-pools/ec2.md), you must fill in the **IAM Instance Profile Name** (not ARN) of the created IAM role when creating the **Node Template**.

While creating a [Custom cluster](./cluster-provisioning/rke-clusters/custom-nodes), you must manually attach the IAM role to the instance(s).
...
When you create an [Amazon EC2 Cluster](./cluster-provisioning/rke-clusters/node-pools/ec2.md), the `ClusterID` is automatically configured for the created nodes. Other resources still need to be tagged manually.
