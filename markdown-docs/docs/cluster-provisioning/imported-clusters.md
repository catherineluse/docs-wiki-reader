...
- [Manage cluster access](./admin-settings/rbac/cluster-project-roles.md) through role-based access control
- Enable [monitoring](./monitoring-alerting/legacy/monitoring/cluster-monitoring.md) and [logging](./cluster-admin/tools/logging.md)
- Enable [Istio](./cluster-admin/tools/istio.md)
- Use [pipelines](./project-admin/pipelines.md)
- Configure [alerts](./cluster-admin/tools/alerts.md) and [notifiers](./cluster-admin/tools/notifiers.md)
- Manage [projects](./project-admin.md) and [workloads](./k8s-in-rancher/workloads.md)

After importing a K3s cluster, the cluster owner can also [upgrade Kubernetes from the Rancher UI.](./cluster-admin/upgrading-kubernetes.md)
...
You can now import a K3s Kubernetes cluster into Rancher. [K3s](./k3s/latest/en.md) is a lightweight, fully compliant Kubernetes distribution. You can also upgrade Kubernetes by editing the K3s cluster in the Rancher UI.
...
To prevent issues when upgrading, the [Kubernetes upgrade best practices](https://kubernetes.io/docs/tasks/administer-cluster/kubeadm/kubeadm-upgrade.md) should be followed.
